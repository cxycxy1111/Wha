...
public class DiscussionTimelineService extends Service{
    ...
    public DiscussionTimelineService() {
        super();
        discussionTimelineDAO =new DiscussionTimelineDAO();
        discussionTimelineUserFavouriteDAO = new DiscussionTimelineUserFavouriteDAO();
    }

    public String add(long timeline_id,long parent_discussion_id,long event_id,long user_id,int user_type,int source,String summary,int anonymous) {
        ...
        if (parent_discussion_id != 0) {
            ...
            list = discussionTimelineDAO.queryByDiscussionId(parent_discussion_id);
            if (Tool.getLongFromArrayList(list,"user_id") == user_id&&Tool.getLongFromArrayList(list,"user_type") == user_type) {
                return FAIL;
            }
        }
        if (discussionTimelineDAO.add(timeline_id,parent_discussion_id,event_id,user_id,user_type,source,summary,anonymous)) {
            return SUCCESS;
        }
        return FAIL;
    }

    public String queryByTimeline(long timeline_id,long user_id,int page_no,boolean login_status) {
        ArrayList<HashMap<String,Object>> list = new ArrayList<>();
        list = discussionTimelineDAO.queryByTimeline(timeline_id,page_no);
        ...
        if (!login_status) {
            ...
            if (list.size() > 0) {
                ...
                list_favourite_list = discussionTimelineUserFavouriteDAO.queryByDiscussionIds(user_id,str);
                for (int i = 0;i<list.size();i++) {
                    ...
                    if (list_favourite_list.size()>0) {
                        for (int j = 0;j < list_favourite_list.size();j++) {
                            ...
                            if (String.valueOf(hashMap.get("id")).equals(String.valueOf(hashMap_2.get("discussion_timeline_id")))) {
    ...
    public String queryBySource(int source,int page_no) {
        return Tool.transformFromCollection(discussionTimelineDAO.queryBySource(source,page_no));
    }

    public String report(long discussion_id,int type) {
        try {
            if (discussionTimelineDAO.addReport(discussion_id,type)) {
                return SUCCESS;
            }else {
                return FAIL;
            }
